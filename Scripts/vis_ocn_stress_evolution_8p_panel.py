...
for i in range(len(data['time'][0:-1][::2])):

    print(data['time'][0:-1][::2][i].values)

    # Get the stress fields for the current time step and flatten them
    awo_ocn_stress_ctl = data['awo_ocn_stress'][0:-1][::2][i].values.flatten()
    awo_ocn_stress_exp = data['awo_ws_ocn_stress'][0:-1][::2][i].values.flatten()

    awo_atm_stress_ctl = data['awo_atm_stress'][0:-1][::2][i].values.flatten()
    awo_atm_stress_exp = data['awo_ws_atm_stress'][0:-1][::2][i].values.flatten()

    #Locate Radius of Maximum Wind
    awo_max_dist = get_max_wind_distance(dist, data['wspd_awo_qc'][0:-1][::2][i].values.flatten())
    awo_ws_max_dist = get_max_wind_distance(dist, data['wspd_awo_ws_qc'][0:-1][::2][i].values.flatten())
    ...
    ax1.hlines(y=1, xmin=xlim[0], xmax=xlim[1], colors='red', linestyles='--', 
               label='$\\tau_{ocn} = \\tau_{atm}$', lw=lw)
    ax1.vlines(x=awo_max_dist, ymin=0, ymax=ylim[1], colors='k', linestyles='-', lw=lw, label='RMW')
    ...
    ax1.set_title('CTL ' + mid_periods[0:-1][::2][i].strftime('%Y-%m-%d %H'), fontsize=fontsize, pad=labelpad)
    ...
    # EXP (AWO_WS) on the right
    ax2.plot(awo_ws_xedges, awo_ws_stress_ratio_means, color='blue', label='EXP mean', zorder=zorder, lw=lw)
    ax2.scatter(dist, awo_ws_stress_ratio, s=s, color='black', alpha=0.5)
    ax2.plot(awo_ws_xedges, awo_ws_std_plus_mean, color='blue', lw=0.5, label=r'$\pm$ EXP std', linestyle='--', zorder=zorder)
    ax2.plot(awo_ws_xedges, awo_ws_std_minus_mean, color='blue', lw=0.5, linestyle='--', zorder=zorder)
    ax2.hlines(y=1, xmin=xlim[0], xmax=xlim[1], colors='red', linestyles='--', lw=lw)
    ax2.vlines(x=awo_ws_max_dist, ymin=0, ymax=ylim[1], colors='k', linestyles='-', lw=lw)
    ax2.set_xlim(xlim)
    ax2.set_ylim(ylim)
    ax2.set_xlabel('Distance from Storm Center (km)', fontsize=fontsize, labelpad=labelpad)
    ax2.set_ylabel('Stress Ratio ($\\tau_{ocn} / \\tau_{atm}$)', fontsize=fontsize, labelpad=labelpad)
    ax2.set_title('EXP ' +mid_periods[0:-1][::2][i].strftime('%Y-%m-%d %H'), fontsize=fontsize, pad=labelpad)
    ax2.tick_params(axis='both', which='major', labelsize=labelsize)
    ax2.grid(linestyle='--', linewidth=linewidth, alpha=alpha)
    # ax2.legend(fontsize=3, loc='best', framealpha=0.8, shadow=True)
    add_corner_label(ax2, xpos, ypos, awo_ws_labels[i], fontsize=4)

    handles1, labels1 = ax1.get_legend_handles_labels()
    handles2, labels2 = ax2.get_legend_handles_labels()
    fig.legend(handles1 + handles2, labels1 + labels2, loc='upper center', ncol=3, fontsize=4, frameon=True, 
               bbox_to_anchor=(0.5, 1.1), shadow=True, framealpha=0.8)

fig.tight_layout(pad=0.01, w_pad=1, h_pad=0.5)


plt.savefig(PNG + 'fig_ocn_stress_evolution_8_panel_JAMES.png', dpi=300, bbox_inches='tight',
