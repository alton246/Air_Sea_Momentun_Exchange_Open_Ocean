...
# IBTRAC Data
PATH_BT = '/home/orca3/adaley17/data/IbTracs/'
...
add_corner_label(ax1, 0.02, 0.90, '(a)', fontsize=5.5)


# Intensisty
# WSPD
ax2 = plt.subplot2grid(gridsize, (1, 0), colspan=1, rowspan=1)
ax2.plot(earl_bt_data.date[::bt_skip][bt_start_date_index:bt_end_date_index], earl_bt_data.max_pres[::bt_skip][bt_start_date_index:bt_end_date_index], 
         color='k', markersize=markersize, linewidth=lw, linestyle='-', marker='s', label='best track')
ax2.plot(track_awo.time[::mod_skip], track_awo.mslp[::mod_skip], linewidth=lw, color='red', linestyle='-', 
         marker='o', markersize=markersize, label='$CTL$')
ax2.plot(track_awo_ws.time[::mod_skip], track_awo_ws.mslp[::mod_skip], color='cyan', linestyle='-', 
...
# ax2.xaxis.set_major_formatter(mdates.DateFormatter("%m-%d")) #Date Format
ax2.set_ylabel('Min. Sea Level Pressure (hPa)', fontsize=fontlabel_size)
ax2.set_xlabel('Date (mm-dd)', fontsize=fontlabel_size)
add_corner_label(ax2, 0.02, 0.90, '(b)', fontsize=5.5)
ax2.set_yticks(ticks=mslp_yticks)
ax2.set_xticks(ticks = track_awo.time[::24])
ax2.set_ylim([ylim_min, ylim_max]) #Minimum and Max x values
ax2.set_xlim([track_awo.time[::mod_skip][awo_start_date_index], 
              track_awo.time[::mod_skip][awo_end_date_index]]) #Minimum and Max x values 
ax2.tick_params(axis='both', which='major', labelsize=fontlabel_size)
ax2.legend(loc='lower left', prop = {"size": 5}, frameon=True, fancybox=True, shadow=True) #Legend
ax2.grid(linestyle=':')


plt.savefig(PNG + 'track_mslp_intensity.png', dpi=300, bbox_inches='tight',
