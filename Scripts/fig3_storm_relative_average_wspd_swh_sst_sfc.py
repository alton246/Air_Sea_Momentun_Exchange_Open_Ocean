...
awo_csv_file_path = os.path.join(Data_Dir, awo_track_file)
awo_ws_csv_file_path = os.path.join(Data_Dir, awo_ws_track_file)

# Read the CSV file into a DataFrame
awo_track_df = pd.read_csv(awo_csv_file_path) #AWO
awo_ws_track_file = pd.read_csv(awo_ws_csv_file_path) #AWO_ws
...
awo_ws_r64, awo_ws_r50, awo_ws_r34 = compute_max_wind_radii(awo_ws_track_file, time_of_int)
...
#Wind Radii
CS_34 = plt.contour(R * np.cos(theta), R * np.sin(theta), R, levels=[awo_max_r34], colors='white', linewidths=0.5)
CS_50 = plt.contour(R * np.cos(theta), R * np.sin(theta), R, levels=[awo_max_r50], colors='magenta', linewidths=0.5)
CS_64 = plt.contour(R * np.cos(theta), R * np.sin(theta), R, levels=[awo_max_r64], colors='blue', linewidths=0.5)
...
CS_34_ws = plt.contour(R * np.cos(theta), R * np.sin(theta), R, levels=[awo_ws_r34], colors='white', linewidths=0.5)
CS_50_ws = plt.contour(R * np.cos(theta), R * np.sin(theta), R, levels=[awo_ws_r50], colors='magenta', linewidths=0.5)
CS_64_ws = plt.contour(R * np.cos(theta), R * np.sin(theta), R, levels=[awo_max_r64], colors='blue', linewidths=0.5)
...
awo_sfc = ax7.contourf(data['x'], data['y'], data['sfc_awo_qc'][0], 
                        cmap=sfc_cmap,levels=sfc_levels, extend='max')

hcb = fig.colorbar(awo_sfc, shrink=shrink, aspect=aspect, ax=ax7, pad=0.02)
hcb.ax.tick_params(color='k', length=1, width=0.5, labelsize=4, pad=0.002)
...
ax7.set_title('$CTL$ $U$ ($m/s$)', fontsize=fontsize, pad=1)
ax7.set_aspect('equal')

#AWO_ws
ax8 = plt.subplot2grid(gridsize, (2, 1), colspan=1, rowspan=1)

awo_ws_sfc = ax8.contourf(data['x'], data['y'], data['sfc_awo_ws_qc'][0], 
                        cmap=sfc_cmap,levels=sfc_levels, extend='max')


hcb = fig.colorbar(awo_ws_sfc, shrink=shrink, aspect=aspect, ax=ax8, pad=0.02)
hcb.ax.tick_params(color='k', length=1, width=0.5, labelsize=4, pad=0.002)
...
ax8.set_title('$EXP$ $U$ ($m/s$)', fontsize=fontsize, pad=1)
...
ax9.set_title('$CTL$ - $EXP$ $U$ ($m/s$)', fontsize=fontsize, pad=1)
...
add_storm_relative_axis_labels(ax10, fontsize, labelpad, labelsize, ticks[::2], ticks[::2])
add_corner_label(ax10, x_pos, y_pos, '(j)', fontsize=fontsize)
ax10.set_title('$CTL$ $T$ ($^{\circ}C$)', fontsize=fontsize, pad=1)
...
add_storm_relative_axis_labels(ax11, fontsize, labelpad, labelsize, ticks[::2], ticks[::2])
add_corner_label(ax11, x_pos, y_pos, '(k)', fontsize=fontsize)
ax11.set_title('$EXP$ $T$ ($^{\circ}C$)', fontsize=fontsize, pad=1)
...
add_storm_relative_axis_labels(ax12, fontsize, labelpad, labelsize, ticks[::2], ticks[::2])
add_corner_label(ax12, x_pos, y_pos, '(l)', fontsize=fontsize)
ax12.set_title('$CTL$ - $EXP$ $T$ ($^{\circ}C$)', fontsize=fontsize, pad=1)
ax12.set_aspect('equal')

fig.tight_layout(pad=0.025, w_pad=0.025, h_pad=0.0025)

# plt.savefig(PNG + 'storm_relative_average_wspd_swh_sst_sfc_wind_radii.png', dpi=300, bbox_inches='tight',
