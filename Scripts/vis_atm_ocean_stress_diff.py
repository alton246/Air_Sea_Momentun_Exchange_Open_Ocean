...
data['atm_stress_diff'] = data['awo_atm_stress'] - data['awo_ws_atm_stress']
data['ocn_stress_diff'] = data['awo_ocn_stress'] - data['awo_ws_ocn_stress']

data['awo_land_mask'] = data['atm_stress_diff'].where(~np.isnan(data['atm_stress_diff']), 999)
data['awo_ws_land_mask'] = data['ocn_stress_diff'].where(~np.isnan(data['ocn_stress_diff']), 999)
...
factors = [
    ('atm_stress_diff', 1, '$ATM$'),
    ('ocn_stress_diff', 1, '$OCN$'),
    ('atm_stress_diff', 3, '$ATM$'),
    ('ocn_stress_diff', 3, '$OCN$'),
    ('atm_stress_diff', 5, '$ATM$'),
    ('ocn_stress_diff', 5, '$OCN$'),
    ('atm_stress_diff', 7, '$ATM$'),
    ('ocn_stress_diff', 7, '$OCN$'),
]
...
cb.set_label('CTL - EXP Stress Difference (N/m²)', fontsize=6, labelpad=2)


fig.tight_layout(pad=0, w_pad=-10, h_pad=0.5)

plt.savefig(PNG + 'atm_ocean_stress_diff.png', dpi=300, bbox_inches='tight',
