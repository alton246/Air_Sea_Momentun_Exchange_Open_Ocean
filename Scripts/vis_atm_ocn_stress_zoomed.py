...
# ticks_zoomed = np.arange(-25,30,5)
...
stress_levels = np.arange(0,0.75,0.05)
...
factors = [
    ('awo_atm_stress', 1, '$\\tau_{atm}$'),
    ('awo_ocn_stress', 1, '$\\tau_{ocn}$'),
    ('awo_atm_stress', 3, '$\\tau_{atm}$'),
    ('awo_ocn_stress', 3, '$\\tau_{ocn}$'),
    ('awo_atm_stress', 5, '$\\tau_{atm}$'),
    ('awo_ocn_stress', 5, '$\\tau_{ocn}$'),
    ('awo_atm_stress', 7, '$\\tau_{atm}$'),
    ('awo_ocn_stress', 7, '$\\tau_{ocn}$')
]
...
cb.set_label('Stress (N/m²)', fontsize=6, labelpad=2)


fig.tight_layout(pad=0, w_pad=-10, h_pad=0.5)

plt.savefig(PNG + 'atm_ocean_stress_zoomed.png', dpi=300, bbox_inches='tight',
